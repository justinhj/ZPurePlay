...
object EvalCatsMTLState extends App {
      ...
      la =>
        eval(r).flatMap {
          ra =>
            val c = la + ra
            S.modify(_ + 1) *>
            L.tell(List(s"Add $la and $ra gave $c")) *> 
            M.pure(c)
        }
    }
  }
          ...
          la =>
            eval(r).flatMap {
              ra =>
                val c = la * ra
                S.modify(_ + 1) *>
                L.tell(List(s"Multiply $la and $ra gave $c")) *> 
                M.pure(c)
            }
    }  
  }
      ...
      la =>
        eval(r).flatMap {
          ra =>
            val c = la / ra
            S.modify(_ + 1) *>
            L.tell(List(s"Div $la by $ra gave $c")) *> 
            M.pure(c)
        }
    }
  }
        ...
        la =>
          eval(r).flatMap {
            ra =>
              val c = la - ra
              S.modify(_ + 1) *>
              L.tell(List(s"Subtract $ra from $la gave $c")) *> 
              M.pure(c)
          }
      }
  }
    ...
    R.ask.flatMap {
      env => 
        env.get(id) match {
          case Some(value) => 
            S.modify(_ + 1) *>
            L.tell(List(s"Var $id was $value")) *> M.pure(value)
  ...
  def eval[F[_],A: Numeric](exp: Exp[A])(implicit L: Tell[F,List[String]], R: Ask[F, Env[A]], E: Raise[F, Error], S: Stateful[F,Int], M: Monad[F]): F[A] = 
      exp match {
        case Val(value) => L.tell(List(s"Pure value $value")) *> S.modify(_ + 1) *> M.pure(value)
  ...
  runResult._2._2 match {
    ...
    case Right(result) => {
      println(s"Computation succeeded with $result after ${runResult._2._1} operaions")
